...
const lib = require(path.resolve(mock.WEBROOT, 'common/lib.js' ));
const Mock = require('mockjs');
const mockExtra = require(path.resolve(mock.WEBROOT, 'common/mock-extra.js' ))
...
module.exports = function(){
  ...
  async  function checkCase(ctx, interfaceId){
    ...
    let caseInst = mock.getInst(caseModel);

    // let ip = ctx.ip.match(/\d+.\d+.\d+.\d+/)[0];
    let ip = mock.commons.getIp(ctx)
    //   数据库信息查询
  ...
  this.bindHook('interface_del', async function(id){
    let inst = mock.getInst(advModel);
  ...
  this.bindHook('mock_after', async function(context){
    ...
    if(caseData){
      ...
      context.mockJson = mock.commons.json_parse(data.res_body);
      try{
    ...
    sandbox = mock.commons.sandbox(sandbox, script);
