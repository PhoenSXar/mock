...
class advMockController extends baseController{
  constructor(ctx){
    super(ctx);
    this.Model = mock.getInst(advModel);
    this.caseModel = mock.getInst(caseModel);
  ...
  async getMock(ctx){
    ...
    if(!mockData){
      return ctx.body = mock.commons.resReturn(null, 408, 'mock脚本不存在');
    }
    return ctx.body = mock.commons.resReturn(mockData);
  }
  ...
  async upMock(ctx){
    ...
    try{
      if(!params.interface_id){
        return ctx.body =mock.commons.resReturn(null, 408, '缺少interface_id');
      }
      if(!params.project_id){
        return ctx.body =mock.commons.resReturn(null, 408, '缺少project_id');
      }
      ...
      return ctx.body = mock.commons.resReturn(null, 400, e.message);
    }
  }
  ...
  async list(ctx){
    let id = ctx.query.interface_id;
    if(!id){
      return ctx.body = mock.commons.resReturn(null, 400, '缺少 interface_id');
    }
  ...
  async getCase(ctx){
    let id = ctx.query.id;
    if(!id){
      return ctx.body = mock.commons.resReturn(null, 400, '缺少 id');
    }
  ...
  async saveCase(ctx){
    ...
    if(!params.res_body){
      return ctx.body =mock.commons.resReturn(null, 408, '请输入 Response Body');
    }
    ...
    if(config.httpCodes.indexOf(data.code) === -1){
      return ctx.body =mock.commons.resReturn(null, 408, '非法的 httpCode');
    }
    ...
    if(findRepeat && findRepeat._id !== params.id){
      return ctx.body = mock.commons.resReturn(null,400, '已存在的期望');
    }
  ...
  async delCase(ctx){
    ...
    if(!id){
      return ctx.body =mock.commons.resReturn(null, 408, '缺少 id');
    }
