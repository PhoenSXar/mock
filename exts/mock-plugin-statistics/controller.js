...
class statisMockController extends baseController {
  constructor(ctx) {
    super(ctx);
    this.Model = mock.getInst(statisMockModel);
  ...
  async getStatisCount(ctx) {
    ...
    return ctx.body = mock.commons.resReturn({ groupCount, projectCount, interfaceCount, interfaceCaseCount });
  }
  ...
  async getMockDateList(ctx) {
    ...
    return ctx.body = mock.commons.resReturn({ mockCount, mockDateList });
  }
  ...
  async getSystemStatus(ctx) {
    let mail = '';
    if (mock.WEBCONFIG.mail && mock.WEBCONFIG.mail.enable) {
    ...
    return ctx.body = mock.commons.resReturn(data);
