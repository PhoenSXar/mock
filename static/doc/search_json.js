window.ydoc_plugin_search_json = {
  "教程": [
    {
      "title": "Lemonce Mock Server",
      "content": "在开始使用 Lemonce Mock Server 之前，我们先来熟悉一下 Lemonce Mock Server 的网站结构，这将让你快速了解Lemonce Mock Server。",
      "url": "/documents/index.html",
      "children": [
        {
          "title": "登录与注册",
          "url": "/documents/index.html#登录与注册",
          "content": "登录与注册想要使用 Lemonce Mock Server ，首先要注册账号。"
        },
        {
          "title": "首页",
      ...
      "children": [
        {
          ...
          "content": "获取权限新用户登录拥有 个人空间 分组下的全部权限，个人空间分组仅自己可见，因此可以在这里任意试用 Lemonce Mock Server 的功能。除此以外没有任何项目或分组的权限，只能浏览已存在分组下面的公开项目。如果找不到想找的项目，可能是尚未成为项目成员，此时应联系 项目组长 将你加入该项目。怎样联系组长？想创建分组，请看: 创建分组\n想创建项目，请看: 创建项目\n想了解更多权限信息，请查看权限列表\n"
        },
        {
          "title": "选择项目",
        ...
        },
        {
          ...
          "content": "新建接口点击左侧接口分组右侧的菜单按钮，选择 添加接口，或者点击接口列表右上角的 添加接口。\n选择接口分类，输入接口名称和接口路径，点击 提交。\n恭喜你！创建了第一个Lemonce Mock Server的接口，你可以看到在左侧看到接口名称，右侧有该接口的信息预览。\n"
        }
      ]
    },
    {
      "title": "权限权利",
      "content": "接口管理的逻辑较为复杂，操作频率高，层层审批将严重拖慢生产效率，因此传统的金字塔管理模式并不适用。Lemonce Mock Server 将扁平化管理模式的思想引入到产品的权限管理中，超级管理员 拥有最高的权限，并将权限分配给若干 组长，超级管理员 只需管理组长 即可，实际上管理Lemonce Mock Server各大分组与项目的是“组长”。组长对分组或项目负责，一般由BU负责人/项目负责人担任。",
      "url": "/documents/manage.html",
      "children": [
        {
          "title": "认识组长",
          "url": "/documents/manage.html#认识组长",
          "content": "认识组长组长分为 分组组长 与 项目组长，他们的关系就是 一个分组内有若干 分组组长 ，这些 分组组长 在创建项目时就可以指定 项目组长。因此他们在职责上的区别就在于 分组组长 对分组负责 项目组长 对项目负责，二者其他具体区别如下：分组组长 的权限包括修改分组、删除分组、创建分组下的项目。一般来说，分组组长 只需要对项目负责，将项目的操作任务安排给 项目组长 处理即可。项目组长 只属于某一个项目因此它无法操作项目所属分组，但拥有项目的全部权限，项目组长 是 Lemonce Mock Server 的基层管理者，承担了 Lemonce Mock Server 绝大部分的日常管理工作。"
        },
        {
          "title": "创建分组",
    ...
    },
    {
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "删除项目点击下方的删除按钮，输入项目名称进行删除。删除项目是高风险操作，因此 Lemonce Mock Server 对这个操作进行了特别的约束。\n"
        },
        {
          "title": "配置环境",
    ...
    {
      "title": "接口设置",
      "content": "进入项目页，可以看到项目下的所有接口，需要注意的是，Lemonce Mock Server有 接口集合 和 测试集合 两个概念。接口集合 将接口进行分类，使结构结构更清晰，一个接口只能属于一个集合，且不允许与其他接口重名。\n测试集合 为了方便我们测试接口，测试集合 将若干接口组合在一起，在这里一个接口可以属于不同集合。\n",
    ...
    {
      "title": "Mock介绍",
      "content": " Lemonce Mock Server的 Mock 功能可以根据用户的输入接口信息如协议、URL、接口名、请求头、请求参数、返回数据（[返回数据](#mock)）生成 Mock 接口，这些接口会自动生成模拟数据，创建者可以自由构造需要的数据。 mock地址解析：Lemonce Mock Server平台网址 + mock + 您的项目id +\b 接口实际请求path假设你 Lemonce Mock Server 的部署地址为：http://Lemonce Mock Server.xxx.com 然后用这个地址作为示例mockd 地址： http://Lemonce Mock Server.xxx.com/mock/29/api/hackathon/login注：项目 id 可以在项目设置里查看到\n",
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "原理 Mock 功能基于 node 和 mockjs，跟 Mockjs 区别是 Lemonce Mock Server 基于 json 定义 mock ，无法使用 mockjs 原有的函数功能，正则表达式需要基于 rule 书写，示例如下：{  \"name|regexp\": \"[a-z0-9_]+?\",\n  \"type|regexp\": \"json|text|xml\" //\b\b枚举数据类型可这样实现\n}\n\n2 支持替换请求的 query, body 参数{  \"name\": \"${query.name}\", //请求的url是/path?name=xiaoming, 返回的name字段是xiaoming\n  \"type\": \"${body.type}\"   //请求的requestBody type=1,返回的type字段是1\n}\n\n其他基本用法请查看：Mockjs 官网"
        },
        {
          "title": "如何使用 Mock",
          "url": "/documents/mock.html#方式2.-json5+注释-如何使用-mock",
          "content": "如何使用 Mock1 在 js 代码直接请求Lemonce Mock Server提供的 mock 地址（不用担心跨域问题）在代码直接请求 Lemonce Mock Server 提供的 mock 地址，以 jQuery 为例：let prefix = 'http://Lemonce Mock Server.xxx.com/mock/2817'$.post(prefix+'/baseapi/path', {username: 'xxx'}, function(res){\n    console.log(res) //返回上图预览部分的数据\n})\n2 \b基于本地服务器反向代理优点:不用修改项目代码2.1 基于 nginx 反向代理location /baseapi{\nproxy_pass   http://Lemonce Mock Server.xxx.com/mock/2817/baseapi; #baseapi后面没有\"/\"\n}\n2.2 基于 ykit mock功能{    pattern: /\\/api\\/(.*)/,\n    responder: 'http://Lemonce Mock Server.xxx.com/mock/58/api/$1'\n}\n上面通过正则匹配，将所有接口转到 http://Lemonce Mock Server.xxx.com 上，比如 http://localhost/api/user/status 会成为 http://Lemonce Mock Server.xxx.com/mock/58/api/user/status详细使用指南: ykit-config-mock2.3 基于 ykit Jerry 代理假设您本地服务器访问地址是： http://xxx.com2.4 基于 Charles 代理点击 Charles 工具栏下的 tools >> Rewrite Settings 填写如下信息："
        },
        {
          "title": "Mock 语法规范",
    ...
    },
    {
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "自定义 Mock 脚本在前端开发阶段，对于某些接口，业务相对复杂，而 UI 端也需要根据接口返回的不同内容去做相应的处理。Lemonce Mock Server 提供了写\b JS 脚本方式处理这一问题，可以根据用户请求的参数修改返回内容。"
        },
        {
          "title": "全局变量",
    ...
    {
      "title": "自动化测试",
      "content": "Web 应用通常是前后端分离开发的，后端提供调用的接口，前端使用接口返回 json 数据渲染到 UI，接口测试就是保证后端接口的数据正确性。对于很多团队，接口测试就是手动运行接口，肉眼比对接口返回的数据，这样的操作流程效率低下，容易出错。使用 Lemonce Mock Server 只需要在可视化 GUI 下，配置下每个接口的入参和对 RESPONSE 断言，即可实现对接口的自动化测试，大大提升了接口测试的效率。自动化测试实践",
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "Mock 参数Mock 参数每次请求都会生成随机字符串变量参数Lemonce Mock Server 提供了强大的变量参数功能，你可以在测试的时候使用前面接口的 参数 或 返回值 作为 后面接口的参数，即使接口之间存在依赖，也可以轻松 一键测试~Tips: 参数只能是测试过程中排在前面的接口中的变量参数\n格式：$.{key}.{params|body}.{path}例如：现有两个接口，分别是“导航标题”和“文章列表”文章列表接口需要传参数: 当前标题(id)，而这个 id 需要通过 导航标题 的返回值获取，这时应在 文章列表 的参数输入框中根据前者的 key 找到对应 id。导航标题 的参数和返回值有如下结构：  参数：\n  \n  返回值：\n  \n则 文章列表 的参数可以如下配置：其中 $. 是使用 动态变量 的标志，$.269.params 即表示 key 值为 269 用例的请求参数，$.269.body 即表示 key 值为 269 用例的返回值。如果 requestBody 是 json 格式也可以在 json 中写变量参数，如下图：Tips: 上下拖动测试集合的列表项可以调整测试的顺序。\n目前 Lemonce Mock Server 中的query，body,header和pathParam的输入参数已经支持点击选择功能。无需自己填写表达式，只需在弹窗中选择需要展示的表达式即可。 输入选项包括常量，mock数据，在测试集合中也支持变量选择。具体用法：单击编辑按钮打开表达式生成器，点击需要的数据创建表达式，这里也可以实时查看表达式结果。Tips: 在测试集合中插入变量参数可以会出现下图的提示信息，这是正常现象。因为该参数只能在各个接口顺序执行的时候才能拉到变量参数中的值\n"
        },
        {
          "title": "自动化测试",
    ...
    {
      "title": "数据导入",
      "content": "在数据管理可快速导入其他格式的接口数据，方便快速添加接口。Lemonce Mock Server 目前支持 postman, swagger, har 数据导入。",
      "url": "/documents/data.html",
      "children": [
        {
          "title": "Postman 数据导入",
          "url": "/documents/data.html#postman-数据导入",
          "content": "Postman 数据导入1.首先在postman导出接口2.选择collection_v1,点击export导出接口到文件xxx3.打开Lemonce Mock Server平台，进入到项目页面，点击数据管理，选择相应的分组和postman导入\b方式，\b选择刚才保存的文件路径，开始导入数据"
        },
        {
          "title": "HAR\b\b 数据导入",
          "url": "/documents/data.html#har\b\b-数据导入",
          "content": "HAR\b\b 数据导入可用 chrome 实现录制接口数据的功能，方便开发者快速导入项目接口1.打开 Chrome 浏览器开发者工具，点击network，首次使用请先clear所有请求信息，确保录制功能开启（红色为开启状态）2.操作页面实际功能，完成后点击save as HAR with content,将数据保存到文件xxx3.打开Lemonce Mock Server平台，进入到项目页面，点击数据管理，选择相应的分组和har导入\b方式，\b选择刚才保存的文件路径，开始导入数据"
        },
        {
          "title": "Swagger 数据导入",
          "url": "/documents/data.html#swagger-数据导入",
          "content": "Swagger 数据导入什么是 Swagger ？[Swagger从入门到精通](https://www.gitbook.com/book/huangwenchao/swagger/details)1.生成 JSON 语言编写的 Swagger API 文档文件  例如这样的数据 （http://petstore.swagger.io/v2/swagger.json），可以将其内容复制到 JSON 文件中。2.打开Lemonce Mock Server平台，进入到项目页面，点击数据管理，选择相应的分组和swagger导入\b方式，\b选择刚才的文件，开始导入数据"
        }
      ]
    },
    {
      ...
      "children": [
        {
          ...
          "content": "安装假设插件名为：Lemonce Mock Server-plugin-demo,安装方法如下：cd {项目目录}Lemonce Mock Server plugin Lemonce Mock Server-plugin-demo\n"
        },
        {
          "title": "卸载插件",
          "url": "/documents/plugin-index.html#卸载插件",
          "content": "卸载插件假设插件名为：Lemonce Mock Server-plugin-demo,卸载方法如下：cd {项目目录}Lemonce Mock Server unplugin Lemonce Mock Server-plugin-demo\n"
        }
      ]
    },
    {
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "加载插件在config.json plugins配置项，加入 demo 插件,{  \"port\": \"3000\",\n  \"db\": {\n    \"servername\": \"127.0.0.1\",\n    \"DATABASE\": \"Lemonce Mock Server\"\n  },\n  ...\n  \"plugins\": [{\n    \"name\": \"demo\"，\n    \"options\": {}\n  }]\n}\n"
        },
        {
          "title": "初始化目录",
          "url": "/documents/plugin-dev.html#初始化目录",
          "content": "初始化目录可参考 项目vendors/exts 目录下的插件在 vendors/node_modules 下新建 Lemonce Mock Server-plugin-demo 目录和 npm init,最后生成的目录接口如下Lemonce Mock Server-plugin-demo  client.js  //客户端入口文件\n  server.js  //服务端入口文件\n  packjson.json //插件依赖管理\n  index.js //插件配置文件\n"
        },
        {
          "title": "index.js 配置说明",
        ...
        {
          "title": "如何使用 Lemonce Mock Server vendors/server 目录下的模块",
          "url": "/documents/plugin-dev.html#server.js-如何使用-Lemonce Mock Server-vendorsserver-目录下的模块",
          "content": "如何使用 Lemonce Mock Server vendors/server 目录下的模块可以直接 require vendors 目录下的模块，注意：后端 node 不能使用 import关键字，只能使用 require例如： require('Lemonce Mock Server')"
        },
        {
      ...
      "children": [
        {
          ...
          "content": "怎么分享我的插件？Fork github.com/ymfe/Lemonce Mock Server ，编辑根目录下的 plugin.json 文件， 然后 Pull-Request 到 ymfe/Lemonce Mock Server"
        },
        {
          "title": "插件列表",
          "url": "/documents/plugin-list.html#插件列表",
          "content": "插件列表window.onload = function(){\n  var list = [{\n    title: 'Lemonce Mock Server-plugin-qsso',\n    url: 'https://github.com/ymfe/Lemonce Mock Server-plugin-qsso',\n    desc: 'qunar 专用 sso 第三方登录'\n  }];\n  var el = $('#list');\n  list.forEach(function(item){\n    el.append(\"\" + '\" + item.title + \"\" + \"&nbsp;\" + item.desc + \"\")\n  })\n}\n"
        }
      ]
    },
    {
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "前端 hookList/** * type component  组件\n *      listener   监听函数\n * mulit 是否绑定多个监听函数\n *\n */\n\nhooks = {\n  /**\n   * 第三方登录 //可参考 Lemonce Mock Server-plugin-qsso 插件\n   */\n  third_login: {\n    type: 'component',\n    mulit: false,\n    listener: null\n  },\n  /**\n   * 导出数据\n   * @param Object exportDataModule\n   * @param projectId\n   * @info\n   * exportDataModule = {};\n   * exportDataModule.pdf = {\n   *   name: 'Pdf',\n   *   route: '/api/plugin/export/pdf',\n   *   desc: '导出项目接口文档为 pdf 文件'\n   * }\n   */\n  export_data: {\n    type: 'listener',\n    mulit: true,\n    listener: []\n  },\n  /**\n   * 导入数据\n   * @param importDataModule\n   *\n   * @info\n   * 可参考 vendors/exts/Lemonce Mock Server-plugin-import-swagger插件\n   * importDataModule = {};\n   *\n   */\n  import_data: {\n    type: 'listener',\n    mulit: true,\n    listener: []\n  },\n  /**\n   * 接口页面 tab 钩子\n   * @param InterfaceTabs\n   *\n   * @info\n   * 可参考 vendors/exts/Lemonce Mock Server-plugin-advanced-mock\n   * let InterfaceTabs = {\n      view: {\n        component: View,\n        name: '预览'\n      },\n      edit: {\n        component: Edit,\n        name: '编辑'\n      },\n      run: {\n        component: Run,\n        name: '运行'\n      }\n    }\n   */\n  interface_tab: {\n    type: 'listener',\n    mulit: true,\n    listener: []\n  }\n};\n"
        }
      ]
    },
    {
    ...
    {
      "title": "常见问题解答",
      "content": "本页面罗列了大家使用 Lemonce Mock Server 时遇到的常见问题.如果没有找到您要的答案，请联系管理员.",
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "Q2 怎么快速迁移旧项目？第一步. 使用 Chrome 浏览器开发者工具录制功能第二步 录制当前项目所有请求，导出到 har 文件第三步 将Har数据导入到 Lemonce Mock Server 平台具体使用方法请参考 Lemonce Mock Server 文档"
        },
        {
          "title": "Q3 忘记密码怎么办？",
        ...
        {
          "title": "Q6 部署Lemonce Mock Server遇到mongodb认证问题？",
          "url": "/documents/qa.html#q6-部署Lemonce Mock Server遇到mongodb认证问题？",
          "content": "Q6 部署Lemonce Mock Server遇到mongodb认证问题？mongodb3.03以上开启认证,解决程序认证连接报错以及第三方客户端无法认证问题\n"
        }
      ]
    }
  ],
  "内网部署": [
    {
      "title": "内网部署",
      "content": "使用我们提供的 Lemonce Mock Server-cli 工具，部署 Lemonce Mock Server 平台是非常容易的。建议部署成 http 站点，因 chrome 浏览器安全限制，部署成 https 会导致测试功能在请求 http 站点时文件上传功能异常。如果您是将服务器代理到 nginx 服务器，请配置 nginx 支持 websocket。在location /添加proxy_http_version 1.1;\nproxy_set_header Upgrade $http_upgrade;\nproxy_set_header Connection \"upgrade\";\n",
      ...
      "children": [
        ...
        },
        {
          ...
          "content": "方式一. 可视化部署[推荐]执行 Lemonce Mock Server server 启动可视化部署程序，输入相应的配置和点击开始部署，就能完成整个网站的部署。部署完成之后，可按照提示信息，执行 node/{网站路径/server/app.js} 启动服务器。在浏览器打开指定url, 点击登录输入您刚才设置的管理员邮箱，默认密码(admin) 登录系统（默认密码可在个人中心修改）。npm install -g Lemonce Mock Server-cli --registry https://registry.npm.taobao.orgLemonce Mock Server server\n"
        },
        {
          "title": "方式二. 命令行部署",
          "url": "/devops/index.html#安装-方式二.-命令行部署",
          "content": "方式二. 命令行部署如果 github 压缩文件无法下载，或需要部署到一些特殊的服务器，可尝试此方法mkdir Lemonce Mock Servercd Lemonce Mock Server\ngit clone https://github.com/YMFE/Lemonce Mock Server.git vendors //或者下载 zip 包解压到 vendors 目录\ncp vendors/config_example.json ./config.json //复制完成后请修改相关配置\ncd vendors\nnpm install --production --registry https://registry.npm.taobao.org\nnpm run install-server //安装程序会初始化数据库索引和管理员账号，管理员账号名可在 config.json 配置\nnode server/app.js //启动服务器后，请访问 127.0.0.1:{config.json配置的端口}，初次运行会有个编译的过程，请耐心等候\n安装后的目录结构如下：|-- config.json|-- init.lock\n|-- log\n`-- vendors\n    |-- CHANGELOG.md\n    |-- LICENSE\n    |-- README.md\n    |-- client\n    |-- common\n    |-- config_example.json\n    |-- doc\n    |-- exts\n    |-- nodemon.json\n    |-- npm-debug.log\n    |-- package.json\n    |-- plugin.json\n    |-- server\n    |-- static\n    |-- test\n    |-- webpack.alias.js\n    |-- Lemonce Mock Server-base-flow.jpg\n    |-- ydocfile.js\n    `-- ykit.config.js\n"
        },
        {
          "title": "服务器管理",
        ...
        },
        {
          ...
          "content": "升级升级项目版本是非常容易的，并且不会影响已有的项目数据，只会同步 vendors 目录下的源码文件。cd  {项目目录}Lemonce Mock Server ls //查看版本号列表\nLemonce Mock Server update //升级到最新版本\nLemonce Mock Server update -v v1.1.0 //升级到指定版本\n"
        },
        {
          "title": "配置邮箱",
